...
describe "Capifony::Symfony2 - symfony" do
  ...
  context "when running symfony:composer:update" do
    ...
    it { should have_run(' sh -c \'cd /var/www/releases/20120927 && curl -sL https://getcomposer.org/installer | php\'') }
  ...
  context "when running symfony:composer:update with a given composer_bin" do
    ...
    it { should_not have_run('vendorDir=/var/www/current/vendor; if [ -d $vendorDir ] || [ -h $vendorDir ]; then cp -a $vendorDir /var/www/releases/20120927; fi;') }
    it { should_not have_run(' sh -c \'cd /var/www/releases/20120927 && curl -sL https://getcomposer.org/installer | php\'') }
  ...
  context "when running symfony:composer:install with an existing composer.phar in the previous release and given composer_version" do
    ...
    it { should_not have_run('vendorDir=/var/www/current/vendor; if [ -d $vendorDir ] || [ -h $vendorDir ]; then cp -a $vendorDir /var/www/releases/20120927; fi;') }
    it { should_not have_run(' sh -c \'cp /var/www/releases/20120920/composer.phar /var/www/releases/20120927/\'') }
    it { should_not have_run(' sh -c \'cd /var/www/releases/20120927 && curl -sL https://getcomposer.org/installer | php -- --version=1.0.0-alpha8\'') }
  ...
  context "when running symfony:composer:install with a given composer_version" do
    ...
    it { should_not have_run('vendorDir=/var/www/current/vendor; if [ -d $vendorDir ] || [ -h $vendorDir ]; then cp -a $vendorDir /var/www/releases/20120927; fi;') }
    it { should have_run(' sh -c \'cd /var/www/releases/20120927 && curl -sL https://getcomposer.org/installer | php -- --version=1.0.0-alpha8\'') }
    it { should have_run(' sh -c \'cd /var/www/releases/20120927 && SYMFONY_ENV=prod php composer.phar install --no-dev --verbose --prefer-dist --optimize-autoloader --no-progress\'') }
  end
